# Fine-tune ResNet-50 on a "pixelated-background" variant of Stanford Cars.
# Same augmentation/baseline recipe as finetune_cars_resnet50.py (RandomResizedCrop + flip).
import argparse
import json
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Fine-tune ResNet-50 on a pixelated-background Cars variant (baseline aug)."
    )
    p.add_argument("--data-root", type=Path, default=Path("/home/jut/pixel"))
    ...
    p.add_argument("--ckpt-dir", type=Path, default=Path("checkpoints_pixel"))
