...
def load_test_annos(anno_path: Path):
    # Test annotations have the same structure; labels are provided in cars_test_annos.mat
...
def train(args):
    ...
    if args.use_test_as_val:
        # Use full train set for training and test labels for validation (matches torchvision example flow).
    ...
    # Match torchvision ImageNet-style aug: RRC + flip for train; resize+center crop for val.
    ...
    # Disable AMP to avoid NaNs on some setups; run full precision.
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Fine-tune ResNet-50 on Stanford Cars (baseline ~90% top-1)."
    )
