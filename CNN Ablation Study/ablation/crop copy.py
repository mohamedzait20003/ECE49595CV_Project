...
# Bbox-aware: pad bbox to square (with optional tolerance), resize to 256, then
# RandomCrop 224 for train (CenterCrop for val). This keeps the object centered
# while preserving the baseline 256→224 workflow.
import argparse
import json
...
def pad_square_crop(img: Image.Image, bbox, tolerance: float, resize_size: int):
    """
    Crop around the bbox, pad to square with black if needed, and resize to resize_size.
    """
    w, h = img.size
    ...
    src_left = max(0, int(round(left)))
    src_top = max(0, int(round(top)))
    src_right = min(w, int(round(right)))
    src_bottom = min(h, int(round(bottom)))
    ...
    return canvas.resize((resize_size, resize_size), Image.BILINEAR)
...
class CarsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = pad_square_crop(img, bbox, self.tolerance, self.resize_size)
        if self.out_size and self.resize_size and self.resize_size != self.out_size:
            if self.center_crop:
                img = transforms.CenterCrop(self.out_size)(img)
            else:
                img = transforms.RandomCrop(self.out_size)(img)
        if self.transform:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Fine-tune ResNet-50 on a bbox-cropped Cars variant (bbox-centered crop + flip)."
    )
