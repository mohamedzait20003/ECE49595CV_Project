# Baseline fine-tuning without any data augmentation (deterministic resize + center crop).
# Mirrors finetune_cars_resnet50.py but removes RandomResizedCrop/flip to serve as a no-aug ablation.
import argparse
import json
...
def train(args):
    ...
    # Deterministic preprocessing: no augmentation (resize + center crop).
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Fine-tune ResNet-50 on Stanford Cars without augmentation (deterministic preprocessing)."
    )
    ...
    p.add_argument("--ckpt-dir", type=Path, default=Path("checkpoints_noaug"))
